...
export default function date() {
     ...
     * @private
     */
    const parseDate = (input: string, inputFormat: string[], separator: string) => {
    ...
    return {
        validate(input: ValidateInput<DateOptions, DateLocalization>): ValidateResult {
            if (input.value === '') {
                return {
                    meta: {
                        date: null,
            ...
            const min = (minOption instanceof Date)
                        ? (minOption as Date)
                        : (minOption ? parseDate(minOption as string, dateFormat, separator) : d);
